  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-ww-bg px-6">
      <div className="w-full max-w-xs text-center">
        ...
        {wrongHost ? (
          <div className="bg-ww-card border border-ww-line rounded-ww-list p-4 text-left">
            <p className="text-sm text-ww-ink2 leading-relaxed mb-3">
              iPhone 上請改用下面這個網址登入，否則 Safari 會擋住 Google 登入。
            </p>
            <a
              href={`https://${HOST_OK}/`}
              className="block w-full text-center bg-ww-brand hover:bg-ww-brandhover text-white
                         py-2.5 rounded-ww-inner text-sm font-medium transition-colors"
            >
              前往正確網址
            </a>
          </div>
          ...
          >
            ...
            <span className="text-sm text-ww-ink font-medium">
        ...
        {error && <p className="mt-5 text-ww-danger text-xs break-words">{error}</p>}
      </div>
    </div>
  );
}
